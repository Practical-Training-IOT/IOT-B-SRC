...
import (
	...
	"github.com/flipped-aurora/gin-vue-admin/server/model/resources_iot/request"
...
func (resourcesApi *ResourcesApi) CreateResources(c *gin.Context) {
	...
	var resources resources_iot.ProtocolConfig
...
func (resourcesApi *ResourcesApi) UpdateResources(c *gin.Context) {
	...
	var resources resources_iot.ProtocolUpdateConfig
	err := c.ShouldBindJSON(&resources)
	if err != nil {
		response.FailWithMessage(err.Error(), c)
		return
...
func (resourcesApi *ResourcesApi) GetResourcesCheck(c *gin.Context) {
	ctx := c.Request.Context()
	var json request.ResourcesSearchRes
	...
	switch json.Name {
	case "HTTP":
		err = resourcesService.CheckHTTP(ctx, json.ID)
		if err != nil {
			response.FailWithMessage("验证失败:"+err.Error(), c)
			return
		}
	case "Kafka":
		err = resourcesService.CheckKafka(ctx, json.ID)
		if err != nil {
			response.FailWithMessage("验证失败:"+err.Error(), c)
			return
		}
	case "MQTT":
		err = resourcesService.CheckMQTT(ctx, json.ID)
		if err != nil {
			response.FailWithMessage("验证失败:"+err.Error(), c)
			return
		}
